...
public class KMeansReduce extends Reducer<DoubleWritable,IntWritable, DoubleWritable,Text>
{
    ...
    public void reduce(DoubleWritable key,Iterable<IntWritable> values,Context context) throws IOException, InterruptedException
    {
        ...
        new_center=Sum/num;
        multipleOutputs.write(new DoubleWritable(Math.round(new_center*10)/10),"","centroid");
        //multipleOutputs.write(new DoubleWritable(Math.abs(key.get()-Math.round(new_center*10)/10)),"","convergence");
        multipleOutputs.write(new DoubleWritable(Math.round(new_center*10)/10), new Text(data),"clustered_data");
    }
